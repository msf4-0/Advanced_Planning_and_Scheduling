...
-- 13. Decoupled Output Schedule Snapshots
CREATE TABLE scheduled_tasks (
    id SERIAL PRIMARY KEY,
    run_id INT NOT NULL REFERENCES scheduling_runs(id) ON DELETE CASCADE,
    operation_id VARCHAR(50) NOT NULL REFERENCES operations(id) ON DELETE CASCADE,
    
    -- Decoupled Output Variables
    optimized_start_minute INT NOT NULL,
    optimized_end_minute INT NOT NULL,
    scheduled_start_time TIMESTAMP NOT NULL,
    scheduled_end_time TIMESTAMP NOT NULL,
    
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uniq_run_operation UNIQUE (run_id, operation_id)
);

COMMENT ON TABLE scheduled_tasks IS 'Stores static runtime snapshots generated by the scheduler, protecting active live operations tables.';
COMMENT ON COLUMN scheduled_tasks.optimized_start_minute IS 'The raw mathematical integer start offset calculated by Google OR-Tools.';
COMMENT ON COLUMN scheduled_tasks.scheduled_start_time IS 'Real calendar datetime timestamp generated matching optimization output timeline.';

-- =========================================================================
-- INDEX OPTIMIZATIONS
-- =========================================================================
CREATE INDEX idx_sched_tasks_run ON scheduled_tasks(run_id);
